...
function SWEP:PlayerHide()
    ...
    Initwalkspeed = self.Owner:GetWalkSpeed()
    Initrunspeed = self.Owner:GetRunSpeed()
    self:EmitSound( HealSound2 )
    self.Owner:SetWalkSpeed(Initwalkspeed*3)
    self.Owner:SetRunSpeed(Initwalkspeed*5)
...
function SWEP:PlayerUnhide()
    ...
    self.Owner:SetWalkSpeed(Initwalkspeed)
    self.Owner:SetRunSpeed(Initrunspeed)
...
if ( CLIENT ) then return end
    ...
    if ( IsValid( ent ) && ( ent:IsPlayer() or ent:IsNPC() )) then
        ...
        pushvector=self.Owner:GetAimVector() * 640
        pushvector.z=100
...
function SWEP:SecondaryAttack()
    ...
    if Hidden == false then self:PlayerHide() else self:PlayerUnhide() end
